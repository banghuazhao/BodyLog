...
struct EditLogEntryView: View {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        if let v = Double(value) {
                            Task { await viewModel.updateEntry(entry, value: v, date: date) }
                        }
                        dismiss()
                    }
                    .fontWeight(.semibold)
    ...
    private func loadCurrentValues() {
        date = entry.date
        if let metric {
            let display = metric.displayValue(entry.value, unitSystem: viewModel.unitSystem)
            value = display.formatted(.number.precision(.fractionLength(1)))
        } else {
            value = entry.value.formatted(.number.precision(.fractionLength(1)))
        }
    }
}
